...
docker run -p 5432:5432 -e POSTGRES_PASSWORD=postgres --name postgres -v "$PWD":/tmp/postgres --restart=always -d -v $PWD/server.crt:/var/lib/postgresql/server.crt:ro -v $PWD/server.key:/var/lib/postgresql/server.key:ro postgres -c ssl=on -c ssl_cert_file=/var/lib/postgresql/server.crt -c ssl_key_file=/var/lib/postgresql/server.key
sleep 5
...
#create app roles
vault write database/config/app3database plugin_name=postgresql-database-plugin allowed_roles="app3" connection_url="postgresql://{{username}}:{{password}}@localhost:5432/" verify_connection=false username="postgres" password="postgres"
vault write database/roles/app3 db_name="app3database" creation_statements="CREATE ROLE \"{{name}}\" WITH SUPERUSER LOGIN PASSWORD '{{password}}' VALID UNTIL '{{expiration}}';" default_ttl="10s" max_ttl="30s"
